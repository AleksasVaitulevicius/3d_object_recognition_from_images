...
image_path_normal = os.getenv('CADNORMALIMGPATH')
...
assert(image_path_normal,'need to set environment variable CADNORMALIMGPATH with the folder which contains the cad models')
...
for m_idx,dir in ipairs(image_list) do
  ...
  assert(render_count==lrender_count,'cad models should have the same number of renders '.. dir)
end
...
for m_idx,dir in ipairs(image_list) do
  ...
  for i,k in ipairs(names) do
    ...
    In = image.load(paths.concat(image_path_normal,dir,k))
    if opt.color == 'gray' then
      I = image.rgb2y(I)
      -- not converting normal images to grayscale
    end
    data[i][1] = image.scale(image_transformer:preprocess(In):float(),224,224)
