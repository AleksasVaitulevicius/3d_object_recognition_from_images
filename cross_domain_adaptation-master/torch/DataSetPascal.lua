...
function DataSetPascal:getGTBoxes(i)
  ...
  for idx0,idx in ipairs(valid_objects) do
    gt_boxes[idx0][1] = math.max(anno.object[idx].bndbox.xmin,1)
    gt_boxes[idx0][2] = math.max(anno.object[idx].bndbox.ymin,1)
    gt_boxes[idx0][3] = math.min(anno.object[idx].bndbox.xmax,anno.size.width)
    gt_boxes[idx0][4] = math.min(anno.object[idx].bndbox.ymax,anno.size.height)
